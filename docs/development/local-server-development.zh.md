...
- P3B 已补齐主要远程 SDK 缺口：批量记录、批量向量写入、`retrieve_context()`、query schema、external_id lookup、受控 import/export 和 snapshot retrieval。
- P3D 已支持逻辑图命名空间：一个物理 SQLite graph/db 内可存多个客户端逻辑图，非管理员数据 API 必须带 `logical_graph_id`。
...
| 逻辑图命名空间 | P3D 已完成 | 一个物理 graph/db 内支持多个 `logical_graph_id`，适合 agent 多图存取 |
| 高级权限 | P3 | per-graph token、角色或更细粒度 logical graph grants |
...
- Live graph API 补齐 `add_nodes()`、`add_edges()`、`get_node_id()`、`query_schema()` 和 `retrieve_context()`。
- Vector API 补齐 `upsert_vectors()` 和 `delete_vectors()`。
- 受控 import/export API：import 只读取 `<data_dir>/imports/**`，export 只写入 `<data_dir>/exports/**`，禁止绝对路径和 `..` 逃逸。
- Import/export 覆盖 nodes/edges CSV、nodes/edges JSONL、nodes/edges JSONL export 和 query rows JSONL export。
- Snapshot retrieval 补齐 fulltext indexes/search、vector indexes/get/search，并保留已有 batch vector search。
...
- 配置和 admin create graph 支持 `logical_graphs: true`，旧字符串路径配置保持兼容。
- 新增 `/graphs/{graph}/logical-graphs` metadata API，write 用户可在已授权物理 graph 中自助创建和删除逻辑图。
- 节点、边、全文、向量、`retrieve_context()`、结构化 query、traversal、algorithm、subgraph、`compute_batch()` 和 snapshot 创建支持 `logical_graph_id`。
- 启用逻辑图模式后，非管理员调用数据 API 必须带 `logical_graph_id`；Cypher 对非管理员返回 `unsupported_operation`。
- Python HTTP client 增加 `RemoteGraph.logical("q450")` 便捷代理，自动传递 `logical_graph_id`。
- P3D 集成测试覆盖逻辑图 metadata、权限、scope 必填、跨逻辑图隔离、检索/查询/算法/snapshot 和 client workflow。

逻辑图命名空间继承物理 graph 的 read/write grant，第一版不做 logical graph 级 grants。
...
当前状态：P1、P2A/P2B/P2C/P2D/P2E、P3A/P3B/P3C/P3D 均已完成。后续主要进入更深的 P3 强化，或根据真实部署反馈补强。
