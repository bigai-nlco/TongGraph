...
- [`Graph`](api/graph.md) is the mutable embedded graph database.
- [`GraphSnapshot`](api/graph-snapshot.md) is a read-only copy of graph state
  used for stable query and compute views.

## Records

- [`Node`](api/node.md) and [`Edge`](api/edge.md) describe property graph
  records returned by lookup methods.
- [`Variable`](api/variable.md), [`Factor`](api/factor.md),
  [`Evidence`](api/evidence.md), and [`Trace`](api/trace.md) describe finite
